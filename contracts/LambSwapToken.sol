...
contract LambSwapToken is ERC20Upgradeable, Initializable, Ownable {
    ...
    bool public traiding = true;

    CustomUsdtDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable USDT = address(0x55d398326f99059fF775485246999027B3197955); // USDT
    address public immutable LPAddress = address(0x55d398326f99059fF775485246999027B3197955); // LPAddress
    address public immutable MarketAddress = address(0x55d398326f99059fF775485246999027B3197955); // MarketAddress

    uint256 public swapTokensAtAmount = 39000000 * (10**9);
    ...
    function initialize() initializer public {
        __ERC20_init("LambSwapToken", "LST");
        dividendTracker = new CustomUsdtDividendTracker();

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        );
        // V2 BSC 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        CustomUsdtDividendTracker newDividendTracker = CustomUsdtDividendTracker(
                payable(newAddress)
            );

        require(
            newDividendTracker.owner() == address(this),
            "The new dividend tracker must be owned by the BabyUsdt token contract"
        );
    ...
    function setTraidingState(bool _state) external onlyOwner {
        traiding = _state;
    }
    ...
    function _transfer(
        ...
        if (!traiding && !_isExcludedFromFees[from]) {
        ...
        if (takeFee) {
            if(from == msg.sender) {
                ...
                super._transfer(from, address(this), fees.div(6));
                super._transfer(from, marketAddress, fees.div(6));
                _burn(from, fees.div(3));
                super._transfer(from, LPAddress, fees.div(3));
            } else {
                uint256 fees = amount.mul(BuyFee).div(100);
                amount = amount.sub(fees);

                super._transfer(from, address(this), fees.div(5));
                super._transfer(from, marketAddress, fees.div(5));
                _burn(from, fees.div(5));
                super._transfer(from, LPAddress, fees.mul(2).div(5));
            }
        }
...
contract CustomUsdtDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    {
        claimWait = 36000;
        minimumTokenBalanceForDividends = 25000 * (10**18); //must hold 200000+ tokens
    }
